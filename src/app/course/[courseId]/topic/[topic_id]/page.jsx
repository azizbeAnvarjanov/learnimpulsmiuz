...
import { ChevronLeft, CircleFadingPlus, Trash } from "lucide-react";
...
const TopicPage = () => {
  ...
  const handleAddQuestion = async () => {
    const newQuestion = {
      ...
      optionC: options[2],
      optionD: options[3],
      answer: options[correct], // To'g'ri javob sifatida tanlangan variant qo'yiladi
    };
    ...
    <div className="p-5 flex gap-4">
      <div className="w-[65%]">
        <div className=" text-2xl font-bold flex items-center gap-2">
          <Link href={`/course/${course_id}`} variant="outline" className="bg-white border  flex items-center justify-center hover:bg-muted w-[45px] h-[45px] rounded-xl"><ChevronLeft /></Link>
          {topic.name}</div>
        ...
        <div className="bg-muted p-3 border mt-4 rounded-xl">
          ...
          <div>{topic.notes.map((item,idx) => (
            <Link key={idx} href={item}>Note fayl</Link>
          ))}</div>
        </div>
      </div>
          ...
          <DialogHeader>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                value={correct}
                onChange={(e) => setCorrect(parseInt(e.target.value - 1, 10))}
              />
              <div className="flex items-center gap-2">
